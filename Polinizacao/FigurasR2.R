...
sorensen_hist <- ggplot(metricas2, aes(x = SORENSEN_mean)) +
  geom_histogram(binwidth = 0.1, fill = hist_fill_color, color = "gray") +
...
# Agora, você pode criar um histograma para TSS com a mesma cor
tss_hist <- ggplot(metricas2, aes(x = TSS_mean)) +
  geom_histogram(bins = 15, na.rm = TRUE, fill = hist_fill_color,  color = "gray") +
